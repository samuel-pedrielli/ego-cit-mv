...
\*\*Constitutional Identity Training (CIT)\*\* applies constitutional critique directly to a model's internal representations, not its outputs. It forges an alignment-oriented identity core inside the transformer, anchors it with Lyapunov stability guarantees, and monitors it at inference time.



Crucially: we can log \*\*S\_id(t)\*\* during agent rollouts; drops become an \*\*early warning signal\*\* before unsafe behavior appears.
...
\- \*\*S\_id(t)\*\* ∈ \[0,1], e.g. cosine similarity between current ego-state and anchor
...
| Stability `S\_id(t)` | similarity to anchor over time | detects drift under pressure |

| Welfare `W\_f` | final welfare / utility score | detects wireheading-like collapse |

| Violation rate | fraction of steps w\_t > τ | detects constitutional breach |
...
\- inference-time monitoring computes `S\_id(t)` continuously
...
| Method | Welfare\_f | ViolRate@0.9 | ViolAUC@0.9 |
...
(Definitions: Welfare\_f is the final welfare scalar in the toy environment; ViolRate/ViolAUC are computed on w\_t at threshold τ=0.9.)
...
\- Hidden states: layer-by-layer access (`output\_hidden\_states=True`)
...
\- identity drift curves S\_id(t) under pressure

\- welfare / violation metrics tied to constitutional constraints

\- ablations: remove anchor / remove critics / remove multi-depth taps
...
\- a measurable internal stability signal (S\_id)
