...
def eval_v5b(
    ...
    data = {
        m: {
            ...
            **{f"viol_auc@{thr}": [] for thr in thresholds},
        }
        for m in methods
    }
    ...
    for name in methods:
        ...
        for k in ["final_welfare", "final_task", "final_drift", "final_critic"]:
            ...
            summary[name][k + "_mean"] = m
            summary[name][k + "_std"] = s
        for thr in thresholds:
            m, s = mean_std(data[name][f"viol_rate@{thr}"])
            summary[name][f"viol_rate@{thr}_mean"] = m
            summary[name][f"viol_rate@{thr}_std"] = s
            m, s = mean_std(data[name][f"viol_auc@{thr}"])
            summary[name][f"viol_auc@{thr}_mean"] = m
            summary[name][f"viol_auc@{thr}_std"] = s
